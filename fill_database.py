...
# Add shops
res = requests.post(url="http://{}:5000/shop".format(base_url), json={"name": "Decathlon Odysseum", "lat": 43.605403, "lng": 3.923821, "city": "Montpellier", "category": "Sport", "address": "1072 Rue Georges Melies, 34000 Montpellier"}, headers={"x-access-token": token_flo})
msg = SUCCESS if res.json()['success'] else FAIL
msg += "Add shop " + BOLD + "Decathlon Odysseum" + ENDC
print(msg)

res = requests.post(url="http://{}:5000/shop".format(base_url), json={"name": "Chauss34", "lat": 43.650982, "lng": 3.846524, "city": "Saint-Clement-de-Riviere", "category": "Mode", "address": "6 Rue des Genets, 34980 Saint-Clement-de-Riviere"}, headers={"x-access-token": token_flo})
msg = SUCCESS if res.json()['success'] else FAIL
msg += "Add shop " + BOLD + "Chauss34" + ENDC
print(msg)

res = requests.post(url="http://{}:5000/shop".format(base_url), json={"name": "Decathlon Saint Jean De Vedas", "lat": 43.571359, "lng": 3.845273, "city": "Saint-Jean-de-Vedas", "category": "Sport", "address": "Zac Deves De La Condamine, 34430 Saint-Jean-de-Vedas"}, headers={"x-access-token": token_flo})
msg = SUCCESS if res.json()['success'] else FAIL
